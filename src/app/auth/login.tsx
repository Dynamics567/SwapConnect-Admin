...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ Check if OTP is required (based on message OR presence of token without full login)
      const isOtpMessage =
  ...
  // useEffect(() => {
  //   if (storedToken) {
  //     console.log("✅ storedToken received, switching to verification...");
  //     setStep("verify");
  //   }
  // }, [storedToken]);
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/admin/complete-2fa`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      // ✅ Save the token returned after OTP verification
      if (data.token) {
        localStorage.setItem("token", data.token);
        router.replace("/dashboard");
      } else {
        setError("No token received. Verification might have failed.");
      }
    } catch (err) {
  ...
  return (
    ...
    ${loading ? "opacity-50 cursor-not-allowed" : "hover:bg-[#026c34]"}`}
            ...
            <form onSubmit={handleVerifyCode} className="w-[720px] max-w-xs">
              <h2 className="text-xl font-bold mb-4 text-[#037F44]">
                Verify Your Email
              </h2>
    ...
    ${loading ? "opacity-50 cursor-not-allowed" : "hover:bg-[#026c34]"}`}
              ...
              <div className="mt-2 text-sm text-gray-500">
                ...
                <button
                  type="button"
                  className="text-[#037F44] underline"
                  onClick={() => {
                    // optionally re-trigger code
                  }}
                >
                  Resend
                </button>
